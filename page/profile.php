...
class page_profile extends Page{
	function init(){
		...
		$empoyement_tab = $tabs->addTab('Curent Job');

		$user= $this->add('Model_Person')->load($this->app->auth->model->id);
		$form  = $profile_tab->add('Form');
		$form->setModel($user,['name','type','email','gender','dob','address','contact_nos','password']);
		...
		$crud->setModel($user->ref('PersonCourse'),['course','from_date','to_date']);

		$emp_form = $empoyement_tab->add('Form');
